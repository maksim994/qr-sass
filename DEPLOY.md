...
> **Если PostgreSQL падает как "unhealthy"**: проверьте, что `POSTGRES_PASSWORD` задан. Пароль не должен содержать символы `@`, `#`, `?` — они ломают URL в `DATABASE_URL`.
...
1. Запустите деплой
2. При старте контейнера `app` выполнится `prisma db push` (создание/синхронизация схемы)
