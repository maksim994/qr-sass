...
export default async function TeamPage() {
  ...
  return (
    <div className="qrs-team-page-wrap">
      <DashboardPageHeader title="Команда" description="Управляйте доступом к общему кабинету." />
      <TeamPageClient
        maxUsers={planInfo.limits.maxUsers}
        workspaceId={workspace.id}
        members={[...members].sort((a, b) => ({ OWNER: 0, ADMIN: 1, MEMBER: 2 }[a.role] - { OWNER: 0, ADMIN: 1, MEMBER: 2 }[b.role])).map((m) => ({
          id: m.id,
          userId: m.user.id,
          email: m.user.email,
          name: m.user.name,
