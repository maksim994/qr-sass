...
export function QrTypesForm({ initialDisabled }: Props) {
  ...
  const [disabled, setDisabled] = useState<Set<QrContentType>>(
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.stack}>
      ...
      >
        Отключённые типы скрываются на странице создания QR-кода. Уже созданные
        QR-коды продолжают работать. Доступно для создания:{" "}
